...
- (void)setupControlView {
    ...
    [self.controlView.tvDLNAButton addTarget:self action:@selector(tvDLNAButtonClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)tvDLNAButtonClick {
    SearchDLNADevice *searching = [SearchDLNADevice view];
    searching.mediaURL = _mediaURL;
    [searching show];
    
    __weak __typeof(self) wself = self;
    searching.dlnaPlayBlock = ^{
        if (wself.player.isPlaying) {
            [wself pause];
        }
    };
}
