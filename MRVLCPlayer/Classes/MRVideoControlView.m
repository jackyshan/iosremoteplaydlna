...
- (void)layoutSubviews
{
    ...
    self.tvDLNAButton.frame   = CGRectMake(CGRectGetWidth(self.bottomBar.bounds) - CGRectGetWidth(self.tvDLNAButton.bounds)*2 - 5, self.playButton.frame.origin.y, CGRectGetWidth(self.tvDLNAButton.bounds), CGRectGetHeight(self.tvDLNAButton.bounds));
...
- (void)setupView {
    ...
    [self.bottomBar addSubview:self.tvDLNAButton];
...
- (UIButton *)tvDLNAButton
{
    if (!_tvDLNAButton) {
        _tvDLNAButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_tvDLNAButton setImage:[UIImage imageNamed:@"tv_dlna"] forState:UIControlStateNormal];
        _tvDLNAButton.bounds = CGRectMake(0, 0, MRVideoControlBarHeight, MRVideoControlBarHeight);
    }
    return _tvDLNAButton;
}
